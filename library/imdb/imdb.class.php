...
#    if (preg_match_all("/\/country\/\w+\"\>(.*?)<\/a/m",$this->page["Title"],$matches))
    ...
    public function crazy_credits() {
        if (empty($this->crazy_credits)) {
            if (empty($this->page["CrazyCredits"]))
                $this->openpage("CrazyCredits");
            if ($this->page["CrazyCredits"] == "cannot open page")
                return array(); // no such page
    ...
    public function episodes() {
        ...
        if (empty($this->season_episodes)) {
            if ($this->page["Episodes"] == "")
                $this->openpage("Episodes");
            if ($this->page["Episodes"] == "cannot open page")
                return array(); // no such page
    ...
    public function goofs() {
        if (empty($this->goofs)) {
            ...
            if (preg_match_all("/<li>.*?<b>(.*?)<\/b>(.*?)<br>\s*(<br>)?<\/li>/ims", $goofs, $matches)) {
                $gc = count($matches[1]);
                for ($i = 0; $i < $gc; ++$i)
                    $this->goofs[] = array("type" => $matches[1][$i], "content" => $matches[2][$i]);
            }
        }
        return $this->goofs;
    }
    ...
    public function quotes() {
        if (empty($this->moviequotes)) {
            if ($this->page["Quotes"] == "")
                $this->openpage("Quotes");
            if ($this->page["Quotes"] == "cannot open page")
                return array(); // no such page
...
// with this, we could even turn it into an array: url,name,quote
    ...
    public function trailers($full = FALSE) {
        if (empty($this->trailers)) {
            if ($this->page["Trailers"] == "")
                $this->openpage("Trailers");
            if ($this->page["Trailers"] == "cannot open page")
                return array(); // no such page
            ...
            if (!empty($tag_s)) { // trailers on the IMDB site itself
                ...
                if (preg_match_all('|<a href="{0,1}(/video/screenplay/vi\d*/).*?title="(.*?)"|ims', $trail, $matches))
                    for ($i = 0; $i < count($matches[0]); ++$i) {
                        ...
                        if ($full)
                            $this->trailers[] = array("lang" => '', "title" => $matches[2][$i], "url" => $trailer, "restful_url" => '');
                        else
    ...
    public function videosites() {
        if (empty($this->video_sites)) {
            if ($this->page["VideoSites"] == "")
                $this->openpage("VideoSites");
            if ($this->page["VideoSites"] == "cannot open page")
                return array(); // no such page
    ...
    public function trivia() {
        if (empty($this->trivia)) {
            if (empty($this->page["Trivia"]))
                $this->openpage("Trivia");
            if ($this->page["Trivia"] == "cannot open page")
                return array(); // no such page
    ...
    public function soundtrack() {
        if (empty($this->soundtracks)) {
            if (empty($this->page["Soundtrack"]))
                $this->openpage("Soundtrack");
            if ($this->page["Soundtrack"] == "cannot open page")
                return array(); // no such page
            if (preg_match('|<p class="disclaimer">(.*?)</script|ims', $this->page["Soundtrack"], $match)) {
                if (preg_match_all("/<li>(.*?)<\/b><br>(.*?)<br>(.*?)<br>.*?<\/li>/", str_replace("\n", " ", $match[1]), $matches)) {
                    $mc = count($matches[0]);
                    for ($i = 0; $i < $mc; ++$i)
                        $this->soundtracks[] = array("soundtrack" => $matches[1][$i], "credits" => array(
    ...
    private function parseConnection($conn) {
        ...
        if (empty($tag_s))
            return array(); // no such feature
        $tag_e = strpos($this->page["MovieConnections"], "<h5>", $tag_s + 4);
        if (empty($tag_e))
            $tag_e = strpos($this->page["MovieConnections"], "<hr/><h3>", $tag_s);
    ...
    public function movieconnection() {
        if (empty($this->movieconnections)) {
            if (empty($this->page["MovieConnections"]))
                $this->openpage("MovieConnections");
            if ($this->page["MovieConnections"] == "cannot open page")
                return array(); // no such page
    ...
    public function extReviews() {
        if (empty($this->extreviews)) {
            if (empty($this->page["ExtReviews"]))
                $this->openpage("ExtReviews");
            if ($this->page["ExtReviews"] == "cannot open page")
                return array(); // no such page
    ...
    public function releaseInfo() {
        if (empty($this->release_info)) {
            if (empty($this->page["ReleaseInfo"]))
                $this->openpage("ReleaseInfo");
            if ($this->page["ReleaseInfo"] == "cannot open page")
                return array(); // no such page
    ...
    public function locations() {
        if (empty($this->locations)) {
            if (empty($this->page['Locations']))
                $this->openpage("Locations");
            if ($this->page['Locations'] == "cannot open page")
                return array(); // no such page
    ...
    public function prodCompany() {
        if (empty($this->compcred_prod)) {
            if (empty($this->page["CompanyCredits"]))
                $this->openpage("CompanyCredits");
            if ($this->page["CompanyCredits"] == "cannot open page")
                return array(); // no such page
    ...
    public function parentalGuide() {
        if (empty($this->parental_guide)) {
            if (empty($this->page["ParentalGuide"]))
                $this->openpage("ParentalGuide");
            if ($this->page["ParentalGuide"] == "cannot open page")
                return array(); // no such page
            if (preg_match_all('/<div class="section">(.*)<div id="swiki(\.\d+\.\d+|_last)">/iUms', $this->page["ParentalGuide"], $matches)) {
                ...
                for ($i = 0; $i < $mc; ++$i) {
                    ...
                    if (empty($sgot))
                        $sgot = $section;
                    switch ($sgot) {
                        case "Alcohol" : $this->parental_guide["Drugs"] = trim($content);
                            break;
                        case "Sex" :
                        case "Violence" :
                        case "Profanity" :
                        case "Frightening":
                        default : $this->parental_guide[$sgot] = trim($content);
                            break;
                    }
                }
            }
    ...
    public function officialSites() {
        if (empty($this->official_sites)) {
            if (empty($this->page["OfficialSites"]))
                $this->openpage("OfficialSites");
            if ($this->page["OfficialSites"] == "cannot open page")
                return array(); // no such page
